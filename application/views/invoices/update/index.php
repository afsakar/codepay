            ...
            hide: function (deleteElement) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $('.inv_subtotal').html(subTotal.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));

                        //Total tax
                        //taxTotal = (sr_price * sr_tax / 100);
                        var totalItemTaxPrice = 0;
                        var itemTaxPriceInput = $('.sr_tax');
                        var taxLength = (itemTaxPriceInput.length) - 1;
                        for (var j = 0; j < taxLength; j++) {
                            totalItemTaxPrice += parseFloat(itemTaxPriceInput[j].value);
                        }
                        $('.inv_total_tax').html(totalItemTaxPrice.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                        $('.inv_total').html((parseFloat(subTotal) + parseFloat(totalItemTaxPrice)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                    }
                })
            },
...
<script>
    ...
    $(document).on('keyup', '.sr_qnt', function () {
        var el = $(this);
        var $sr_amount = $(el.parent().parent().find('.sr_amount')).val();
        var $sr_taxprice = $(el.parent().parent().find('.sr_price')).val();
        var $sr_qnt = $(this).val();
        var $total = parseFloat($sr_qnt * ($sr_amount)).toFixed(2);
        $(el.parent().parent().find( ".item_total" )).html($total);

        var totalItemPrice = ($sr_qnt * $sr_amount);
        var amount = (totalItemPrice);
        $(el.find('.item_total')).html(amount);

        var totalItemTaxRate = $(el.parent().parent().find('.item_tax')).html();
        var itemTaxPrice = parseFloat($sr_qnt * $sr_taxprice);
        $(el.parent().parent().find('.sr_tax')).val(itemTaxPrice.toFixed(2));
        ...
        $('.inv_total_tax').html(totalItemTaxPrice.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
        $('.inv_total').html((parseFloat(subTotal) + parseFloat(totalItemTaxPrice)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'));
        $('#inv_total').val((parseFloat(subTotal) + parseFloat(totalItemTaxPrice)).toFixed(2));
        ...
        $(document).on('change', '.services', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var sr_code = item['sr_code'];
                    var sr_amount = item['sr_amount'];
                    var sr_taxprice = item['sr_price'];
                    var sr_qnt = 0;
                    var sr_tax = item['sr_tax'];
                    var sr_unit = item['sr_unit'];

                    var itemTaxPrice  = parseFloat(sr_qnt * sr_amount * sr_tax / 100);
                    $(el.parent().parent().find( ".sr_code" )).val(sr_code);
                    $(el.parent().parent().find( ".sr_amount" )).val(sr_amount);
                    $(el.parent().parent().find( ".sr_price" )).val(sr_taxprice);
                    $(el.parent().parent().find( ".sr_qnt" )).val(sr_qnt);
                    $(el.parent().parent().find( ".item_tax" )).html(sr_tax);
                    $(el.parent().parent().find( ".sr_tax" )).val(itemTaxPrice );
                    $(el.parent().parent().find( ".sr_unit" )).val(sr_unit);
