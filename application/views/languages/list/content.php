<div class="content">
    <h2 class="content-heading"><?=$title?>
        <?php if(permission("languages", "add")): ?>
            <a href="<?=base_url("languages/addForm")?>" class="btn btn-alt-primary btn-sm pull-right">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-vcenter table-hover table-container js-dataTable-full">
                        ...
                        <tr>
                            ...
                            <th><?=trans("language_code")?></th>
                            <th><?=trans("created_at")?></th>
                            <?php if(permission("languages", "edit")): ?>
                                <th><?=trans("status")?></th>
                            <?php endif; ?>
                            <th class="text-center" style="width: 100px;"><?=trans("actions")?></th>
                        </tr>
                        </thead>
                        <tbody class="sortable" data-url="<?php echo base_url("languages/rankSetter")?>">
                        ...
                        <tr id="ord-<?=$item->id?>">
                            ...
                            <?php if(permission("languages", "edit")): ?>
                                <td>
                                    <label class="css-control css-control-sm css-control-primary css-switch">
                                        <input type="checkbox" class="css-control-input isActive" id="block-form-remember-me<?=$item->id?>" data-url="<?php echo base_url("languages/isActiveSetter/$item->id")?>" <?=$item->isActive == 1 ? 'checked' : null?>>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <?php if(permission("languages", "edit")): ?>
                                        <a href="<?php echo base_url("languages/wordsForm/$item->id")?>" type="button" class="btn btn-sm btn-secondary">
                                                <i class="fa fa-language"></i>
                                        </a>
                                        <a href="<?php echo base_url("languages/updateForm/$item->id")?>" type="button" class="btn btn-sm btn-secondary">
                                            <i class="fa fa-pencil"></i>
                                        </a>
                                    <?php endif; ?>
                                    <?php if(permission("languages", "delete")): ?>
                                        <button data-url="<?php echo base_url("languages/deleteItem/$item->id")?>"
